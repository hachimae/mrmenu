...
class Auth extends MY_Controller
{
	...
	function register()
	{
		if ($this->auth->is_logged_in()) {									// logged in
			...
			$this->form_validation->set_rules('shopname', 'Shop name', 'trim|required|xss_clean');
			$this->form_validation->set_rules('shopdescription', 'Shop description', 'trim|required|xss_clean');
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->auth->create_user(
					...
					$this->db->update('ci_user_profiles', array(
					...
					$this->db->insert('ci_shop', array(
						'owner_id' => $data['user_id'],
						'name' => $this->form_validation->set_value('shopname'),
						'detail' => $this->form_validation->set_value('shopdescription'),
					...
					if ($email_activation) {									// send "activate" email
						...
						$this->_show_message($this->lang->line('auth_message_registration_completed_2').' '.anchor('/app/login/', 'Login'));
					}
				} else {
					$errors = $this->auth->get_error_message();
					foreach ($errors as $k => $v)	$data['errors'][$k] = $this->lang->line($v);
	...
	function activate()
	{
		...
		if ($this->auth->activate_user($user_id, $new_email_key)) {		// success
			$this->auth->logout();
			$this->_show_message($this->lang->line('auth_message_activation_completed').' '.anchor('/app/login/', 'Login'));
	...
	function reset_password()
	{
		...
		if ($this->form_validation->run()) {								// validation ok
			if (!is_null($data = $this->auth->reset_password(
				...
				$this->_show_message($this->lang->line('auth_message_new_password_activated').' '.anchor('/app/login/', 'Login'));
	...
	function reset_email()
	{
		...
		if ($this->auth->activate_new_email($user_id, $new_email_key)) {	// success
			$this->auth->logout();
			$this->_show_message($this->lang->line('auth_message_new_email_activated').' '.anchor('/app/login/', 'Login'));
