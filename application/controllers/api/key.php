...
class Key extends REST_Controller
{
	...
	public function index_put()
    {
        ...
        // check for the mobile ID
        $mobile_id = $this->put('mobile_id');
        if( empty($mobile_id) )
        {
            $this->response(array('status' => 0, 'error' => 'Could not find Mobile ID.'), 500);
        }
        else
        {
            ...
            {
                // insert key and mobile into relation table
                if( self::_create_client($key, $mobile_id) )
                {
                    $this->response(array('status' => 1, 'key' => $key), 201); // 201 = Created
                }
                else
                {
                    $this->response(array('status' => 0, 'error' => 'Duplicate mobile device.'), 500);
                }
            }
            else
            {
    ...
    private function _create_client($key, $mobile_id)
    {
        $data['api_key'] = $key;
        $data['device_key'] = $mobile_id;
        
        // check dupplicate
        if( $this->rest->db->where('device_key = ', $mobile_id)->count_all_results('ci_member') ){
            return false;
        }
        
        // insert key
        return $this->rest->db->set($data)->insert('ci_member');
    }
